    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('games.play', [$game->id, $l->id]) }}"
           class="w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold transition-all
                  {{ $l->id === $level->id ? 'bg-green-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-green-100 dark:hover:bg-green-900' }}">
            {{ $l->level_number }}
        </a>
        @endforeach
    </div>
...
@livewire('games.typing-race', ['level' => $level])
